...
export async function parseMinimalDslToQuery(dsl: MinimalDSL): Promise<Query> {
  ...
  if (dsl.joins && dsl.joins.length > 0) {
    for (const j of dsl.joins) {
      ...
      const conditions = (j.on || []).map(o => new JoinCondition({ leftField: o.left, rightField: o.right }));
      joins.push(new Join({ type: (j.type || 'left') as any, leftTable: mainTable, rightTable: right, conditions }));
    }
  }
  ...
  const basicMetrics = (dsl.metrics || []).filter(m => !['post_agg', 'arithmetic'].includes(m.type));

  for (const m of basicMetrics) {
    const name = m.name || (m.field || m.type);
    if (m.type === 'subquery') {
      // use SubQueryMetric via sqlTemplate
      metricsMap[name] = new SubQueryMetric(name, m.sqlTemplate || '', {}, undefined, m.alias);
      continue;
    }
    ...
    if (m.type === 'row') {
      ...
      if (!match) {
        throw new Error('row metric 目前仅支持简单二元运算表达式，例如 "{unit_price} / {quantity}"');
      }
      ...
      const opMap: Record<string, Operator> = { '+': Operator.PLUS, '-': Operator.MINUS, '*': Operator.MULTIPLY, '/': Operator.DIVIDE };
    ...
    if (!m.field) {
      ...
      if (!idField) throw new Error('缺少用于 count 的字段，请在 metric 中指定 field');
      metricsMap[name] = new AggregateMetric(name, func, idField, isDistinct, m.alias);
    } else {
      const f = resolveField(m.field);
      metricsMap[name] = new AggregateMetric(name, func, f, isDistinct, m.alias);
    }
  }
  ...
  for (const m of dsl.metrics || []) {
    if (['post_agg','arithmetic'].includes(m.type)) {
      const name = m.name || (m.field || m.type);
      if (m.type === 'post_agg') {
        if (!m.agg || !m.metric) throw new Error('post_agg 需要 agg 和 metric 字段');
        ...
        metricsMap[name] = new PostAggregateMetric(name, func, refMetric, false, m.alias);
      } else if (m.type === 'arithmetic') {
        // left/right can be { metric: 'name' } or { field: 'table.field' } or number
        if (!m.left || !m.operator || (m.right === undefined)) throw new Error('arithmetic 需要 left/operator/right');
        ...
        const opMap: any = { '+': Operator.PLUS, '-': Operator.MINUS, '*': Operator.MULTIPLY, '/': Operator.DIVIDE };
        const operatorEnum = opMap[opSymbol] || Operator.PLUS;
        metricsMap[name] = new ArithmeticMetric(name, leftOperand, operatorEnum, rightOperand, m.alias);
      }
    }
  }
  ...
  const filters = (dsl.filters || []).map(f => {
    ...
    if (f.op === 'recent_days' && typeof f.value === 'number') {
      return TimeFilter.createRecentFilter(fieldObj, TimeRange.RECENT_DAYS, f.value);
    }
