 ...
 *
 * 本库提供以下核心功能：
 * - V2 表达式 AST 系统（v2/expr 模块）：统一的表达式抽象语法树
 * - V2 SQL 构建器（v2/sql 模块）：基于 Knex 的链式 SQL 构建
 * - 兼容层（compat 模块）：新旧 API 适配器
 *
 * ## 架构说明
 *
 * ### V2 新架构（推荐使用）
 * - `v2/expr` 模块：表达式 AST，支持字段引用、聚合函数、算术运算
 * - `v2/sql` 模块：Knex 链式 SQL 构建，自动处理 CTE
 * - `compat` 模块：兼容旧 API，无需修改现有代码
 *
 * ### V1 旧架构（已废弃）
 * - `v1/metrics/metric-classes.ts`：旧的指标类系统
 * - `v1/query/query-builder.ts`：旧的查询构建器
 *
...
// 注意：core/types 中的部分类型与 v2/expr/types 重复
// 这里显式导出需要的类型，避免冲突
export {
  DatabaseClient,
  DatabaseDialect,
...
/**
 * @deprecated 使用 `v2/expr` 模块中的表达式类替代
 *
 * 迁移指南：
 ...
 */
export * from "./v1/metrics/metric-classes";

/**
 * @deprecated 使用 `v2/sql` 模块中的 QuerySpec 和 KnexQueryBuilder 替代
 *
 * 迁移指南：
 * - Query → QuerySpec
 ...
 */
export * from "./v1/query/query-builder";

/**
 * @deprecated 使用 `v2/expr` 模块中的表达式系统替代
 */
export * from "./v1/query/filter";

/**
 * @deprecated 使用 `v2/sql` 模块中的 KnexQueryBuilder 替代
 *
 * 注意：KnexSQLGenerator 内部已使用新架构实现，
 * 现有代码无需修改即可使用新架构。
 */
export * from "./v1/query/knex-sql-generator";

/**
 * @deprecated 使用 `v2/expr` 模块替代
 */
export * from "./v1/query/alias-registry";
