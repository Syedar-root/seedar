...
Dashboard 模块是一个用于创建、管理和展示仪表盘的功能模块，支持用户通过可视化方式组织和展示数据。该模块允许用户创建多个仪表盘，每个仪表盘可以包含多种类型的面板，如图表、表格、文本和卡片，以满足不同的数据展示需求。布局支持响应式设计，适配不同屏幕尺寸。

### 1.1 核心价值

- **数据可视化**：通过多种面板类型，直观展示数据
- **灵活配置**：支持自定义仪表盘布局和面板配置
- **响应式设计**：支持多断点布局，适配不同屏幕尺寸
- **模块化设计**：支持面板的复用和共享
...
| 功能 | 描述 |
|------|------|
| 更新布局 | 更新仪表盘的布局配置，支持响应式断点 |
| 布局验证 | 验证布局中的面板 ID 是否存在 |
...
> 【更新于 2026-03-15】：根据代码变更，新增响应式布局设计章节，更新布局结构为响应式布局格式（支持 lg/md/sm/xs/xxs 断点），新增布局验证功能，修正关联表名称为 `dashboard_panels`，新增布局验证边界约束。
