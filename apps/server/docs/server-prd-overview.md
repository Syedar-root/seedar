...
Seedar 后端服务是一个基于 NestJS 框架开发的数据分析平台后端，提供完整的数据源管理、数据集构建、查询执行和仪表盘展示能力。
...
**功能描述**：管理和展示仪表盘，支持添加不同类型的面板，并支持响应式布局。
...
| 布局管理 | 更新仪表盘布局（支持响应式断点） | 高 |
| 面板关联 | 向仪表盘添加或移除面板 | 高 |
| 布局验证 | 验证布局中的面板 ID 是否有效 | 高 |
...
4. **仪表盘创建流程**：创建仪表盘 → 创建面板 → 添加面板到仪表盘 → 配置响应式布局
...
| Dashboard | 仪表盘基本信息 |
| Panel | 面板信息 |
| DashboardPanelRelation | 仪表盘与面板的关联关系 |
...
Seedar 后端服务通过四个核心模块的协作，提供了完整的数据分析平台能力。数据源模块作为基础组件管理各种数据源，数据集模块在其上构建数据模型，查询模块实现数据分析功能，仪表盘模块提供数据可视化展示。
...
> 【更新于 2026-03-15】：新增仪表盘模块 PRD 说明，包含核心功能、面板类型、响应式布局，更新典型业务流程图，新增仪表盘加载性能需求，更新验收标准。
